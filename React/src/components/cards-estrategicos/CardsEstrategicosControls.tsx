...
interface CardsEstrategicosControlsProps {
  empresaId: string | null;
  onEmpresaChange: (empresaId: string | null) => void;
  onRefresh: () => void;
}

export function CardsEstrategicosControls({ 
  empresaId, 
  onEmpresaChange, 
  onRefresh 
}: CardsEstrategicosControlsProps) {
  return (
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
          ...
          {!empresaId && (
            <p className="text-sm text-muted-foreground lg:absolute lg:left-1/2 lg:transform lg:-translate-x-1/2">
              Selecione uma empresa para visualizar cards estratégicos
            </p>
          )}
