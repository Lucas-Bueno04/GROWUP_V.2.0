...
export default function OrcamentosPage() {
  ...
  const fetchData = async () => {
    ...
    // Pega grupos com contas - só precisa se quiser usar antes do createBudget
    try {
      const groups = await getAllGroupsWithAccounts(token);
      console.log("Grupos ao carregar a página:", groups);
      // Você pode armazenar esses grupos em um estado, se quiser usar depois
      // Exemplo:
      // setGroups(groups);
    } catch (err) {
      console.error("Erro ao buscar grupos no carregamento:", err);
    }
  };
