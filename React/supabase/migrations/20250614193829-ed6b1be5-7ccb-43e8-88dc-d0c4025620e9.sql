...
-- Corrigir a função calcular_indicador_painel_empresa para resolver ambiguidade de colunas
...
BEGIN
  ...
  WHILE formula_processada ~ 'G\d+' LOOP
    ...
    -- Calcular valor do grupo - CORRIGIDO: especificar explicitamente a tabela para a coluna sinal
    SELECT COALESCE(SUM(
      CASE 
        WHEN og.sinal = '+' THEN oev.valor_realizado
  ...
  WHILE formula_processada ~ 'C\.[0-9\.]+' LOOP
    ...
    -- Buscar valor acumulado da conta até o mês especificado - CORRIGIDO: especificar explicitamente a tabela para a coluna sinal
    SELECT COALESCE(SUM(
      CASE 
